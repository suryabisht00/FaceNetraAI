            ...
            {isAuthenticated ? (
              <li className="relative profile-menu-container">
                <button
                  ...
                  className="w-10 h-10 rounded-full bg-primary/20 border-2 border-primary/40 flex items-center justify-center hover:bg-primary/30 transition-all"
                >
                ...
                {/* Dropdown Menu */}
                {showProfileMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-[#0B0F1A] border border-primary/20 rounded-xl shadow-lg overflow-hidden z-50">
                    <button
                      onClick={handleProfileClick}
                      className="w-full px-4 py-3 text-left text-sm text-white hover:bg-primary/10 transition-all flex items-center gap-3"
                    >
                    ...
                    <button
                      ...
                      className="w-full px-4 py-3 text-left text-sm text-red-400 hover:bg-red-500/10 transition-all flex items-center gap-3"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center gap-3">
            {!isAuthenticated ? (
              ...
              <div className="relative profile-menu-container">
                <button 
                  onClick={() => setShowProfileMenu(!showProfileMenu)}
                  className="w-10 h-10 rounded-full bg-primary/20 border-2 border-primary/40 flex items-center justify-center hover:bg-primary/30 transition-all active:scale-95"
                >
                  <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  </svg>
                ...
                {showProfileMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-[#0B0F1A] border border-primary/20 rounded-xl shadow-lg overflow-hidden z-50 backdrop-blur-md">
                    <button
                      onClick={handleProfileClick}
                      className="w-full px-4 py-3 text-left text-sm text-white hover:bg-primary/10 transition-all flex items-center gap-3"
                    >
